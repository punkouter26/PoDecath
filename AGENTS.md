...
- **A commentary voice for Mac, Linux and iOS.** `SpeechSynth` has real voices only on Android and
  Windows and this is unchanged. sherpa-onnx was the recommendation and it turns out to ship **no Unity
  integration at all** (the repo has bindings for a dozen languages; Unity is not among them), so it
  means hand-writing P/Invoke against its C API and carrying native libraries per platform, roughly
  16 MB for Windows and 46 MB for Android, plus a voice model. That is a project, not an install.
  The cheaper route worth trying first is a character-tokenised VITS voice run straight through
  `com.unity.ai.inference`, which this project already ships for the policies: no native library, no
  new runtime, and the text-to-token step becomes ordinary C#. Piper's own voices will not take that
  route because they are phonemised by espeak-ng, which is itself native.
- **Replacing the synthesised audio.** Kenney's CC0 libraries were the suggestion and they do not fit
  this bank: they are interface, impact, digital and sci-fi sets for casual games, and `AudioBank`
  wants a crowd bed, a starting pistol, a lap bell, a hurdle clatter and footfalls on track, sand and
  rubber. Freesound has all of it, but it needs an account and each sound has to be licence-checked
  one at a time (much of it is CC-BY, not CC0), so it is an owner decision rather than a scripted step.
